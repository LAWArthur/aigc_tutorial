...
def train_step_with_rng_control(train_step_fn, model_without_ddp, step: int, base_seed: int, samples: torch.Tensor, class_labels: torch.Tensor, aug_cond):
    rank = get_rank()
    ...
    with torch.random.fork_rng(devices=[input_device], enabled=True):
        ...
        return train_step_fn(model_without_ddp, samples, class_labels, aug_cond)
