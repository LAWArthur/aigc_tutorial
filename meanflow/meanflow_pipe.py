...
from utils.ema import init_ema, update_ema_net
...
class MeanFlow(nn.Module):
    def __init__(self, args):
        ...
        # setup model ema
        self.model_ema = init_ema(self.model, copy.deepcopy(self.model), args.ema_decay)

        # maintain extra ema nets
        self.ema_decays = args.ema_decays
        for i, ema_decay in enumerate(self.ema_decays):
            self.add_module(f"model_ema{i + 1}", init_ema(self.model, copy.deepcopy(self.model), ema_decay))
    ...
    def update_ema(self):
        ...
        update_ema_net(self.model, self.model_ema, num_updates)

        # update extra ema
        for i in range(len(self.ema_decays)):
            update_ema_net(self.model, self._modules[f"model_ema{i + 1}"], num_updates)

    def forward_with_loss(self, x, class_labels, aug_cond):
        ...
        def u_func(z, t, r):
            h = t - r
            return self.model(z, (t.view(-1), h.view(-1)), class_labels=class_labels, aug_cond=aug_cond)
    ...
    def sample(self, samples_shape, model=None, device=None, class_labels=None):
        model = model if model is not None else self.model_ema                
        ...
        u = model(z_1, (t, t - r), aug_cond=None, class_labels=class_labels)
        z_0 = z_1 - u
        return z_0

def instantiate_meanflow(args) -> nn.Module:
    return MeanFlow(args)
